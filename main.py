...
def add_reminder_button():
    keyboard = [[KeyboardButton(ADD_REMINDER_TEXT)]] # keyboard button returns an object within the list
    return ReplyKeyboardMarkup(keyboard) # expects 2D list
...
def enter_message_handler(update: Update, context: CallbackContext):
    update.message.reply_text("Please enter a time bot should remind (format: DD/M/YYYY hh:mm : ")
